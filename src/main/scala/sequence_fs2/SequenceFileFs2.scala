...
object SequenceFileFs2 {
  ...
  implicit val bytesWritableProvider: WritableProvider[ByteBuffer] = new WritableProvider[ByteBuffer] {
    override def getEmptyWritable: Writable = new BytesWritable()

    override def getWritable(t: ByteBuffer): Writable = ???
  ...
  implicit def arrayWritableProvider[T: WritableProvider : ClassTag]: WritableProvider[Array[T]] = new WritableProvider[Array[T]] {
    ...
    override def getWritable(t: Array[T]): Writable = {
      ...
      new ArrayWritable(innerWritableProvider.getEmptyWritable.getClass, t.map(innerWritableProvider.getWritable))
    }
  ...
  implicit def mapWritableProvider[K: WritableProvider : ClassTag, V: WritableProvider : ClassTag]: WritableProvider[Map[K, V]] = new WritableProvider[Map[K, V]] {
    ...
    override def getValue(w: Writable): Map[K, V] = {
      ...
      mapWritable.keySet().asScala.map { k =>
  ...
  /**
   * Creates an [[fs2.Stream]] that reads from the specified Sequence File.
   * The key and values of the records need to have an implicit [[WritableProvider]] in scope, that can wrap its values
   * into a hadoop Writable.
   *
   * @param filename   the file path of the Sequence File
   * @param start      the number of bytes to skip
   * @param length     the amount of bytes to read
   * @param bufferSize the buffer size of the internal reader
   * @param config     Hadoop Configuration for the internal reader
   * @tparam F effect type
   * @tparam K type of the record key
   * @tparam V type of the record value
   * @return [[fs2.Stream]] of records of type (K, V)
   */
      ...
      Stream.unfoldEval(reader) { _ =>
        Sync[F].blocking {
          ...
          if (reader.next(kWritable, vWritable)) {
            Option(((kWritableProvider.getValue(kWritable), vWritableProvider.getValue(vWritable)), reader))
          } else {
            None
          }
        }
  ...
  /**
   * Creates an [[fs2.Pipe]] that writes data to a single Sequence File.
   * The key and values of the records need to have an implicit [[WritableProvider]] in scope, that can wrap its values
   * into a hadoop Writable.
   *
   * @param filename        the file path of the Sequence File
   * @param compressionType one of NONE, BLOCK, RECORD compression types
   * @param config          Hadoop Configuration for the internal writer
   * @tparam F effect type
   * @tparam K type of the record key
   * @tparam V type of the record value
   * @return writer [[fs2.Pipe]]
   */
  def write[F[_] : Sync, K: WritableProvider, V: WritableProvider](filename: String, compressionType: CompressionType = CompressionType.NONE, config: Configuration = new Configuration()): Pipe[F, (K, V), (K, V)] = {
    in => {
